...
const listDiseases = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Diseases fetched', data });
  } catch (error) {
    return next(error);
  }
};
...
const addDisease = async (req, res, next) => {
  try {
    ...
    await del(CACHE_KEY);
    await logAuditAction({ userId: req.user.id, role: req.user.role, action: 'CREATE', entityType: 'disease', entityId: result.rows[0].id, details: { disease_name: result.rows[0].disease_name } });
    return res.status(201).json({ success: true, message: 'Disease info added', data: result.rows[0] });
  } catch (error) {
    if (error.code === '23505') return next({ statusCode: 400, message: 'Disease already exists' });
    return next(error);
  }
};
...
const updateDisease = async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE diseases SET disease_name=$1, symptoms=$2, prevention=$3, treatment=$4, risk_factors=$5 WHERE id=$6 RETURNING *`,
    ...
    await del(CACHE_KEY);
    await logAuditAction({ userId: req.user.id, role: req.user.role, action: 'UPDATE', entityType: 'disease', entityId: result.rows[0].id, details: { disease_name: result.rows[0].disease_name } });
...
const deleteDisease = async (req, res, next) => {
  try {
    ...
    if (result.rowCount === 0) return next({ statusCode: 404, message: 'Disease not found' });
    await del(CACHE_KEY);
    await logAuditAction({ userId: req.user.id, role: req.user.role, action: 'DELETE', entityType: 'disease', entityId: req.params.id });
